...
def helpMessage() {
    ...
    References                          If not specified in the configuration file or you wish to overwrite any of the references
      ...
      --fastp [bool]                    Use Fasp for adapter trimming (Defaul: false)
      --align_unpaired [bool]           Align unpaired reads from TRIMMOMATIC (Default: false)
...
if (!params.skip_adapter_trimming && params.fastp) {
    process FASTP {
        ...
        tuple val(sample), val(single_end), path("*.trim.fastq.gz") into ch_fastp_trimmomatic_bowtie2,
                                                                         ch_fastp_trimmomatic_cutadapt,
                                                                         ch_fastp_trimmomatic_kraken2
...
process BOWTIE2 {
    ...
    if (single_end) {
        ...
        if (params.align_unpaired) {
            input_reads = "-1 ${reads[0]} -2 ${reads[1]} -U ${reads[2]},${reads[3]}"
        } else {
...
if (params.protocol == 'amplicon' && !params.skip_assembly && !params.skip_amplicon_trimming) {
    process CUTADAPT {
        ...
        input:
        tuple val(sample), val(single_end), path(reads) from ch_fastp_trimmomatic_cutadapt
        ...
        my_reads = single_end ? "$reads" : "${reads[0]} ${reads[1]}"
        """
    ...
    ch_fastp_trimmomatic_kraken2 = ch_cutadapt_kraken2
...
if (!params.skip_kraken2 && !params.skip_assembly) {
    process KRAKEN2 {
        ...
        input:
        tuple val(sample), val(single_end), path(reads) from ch_fastp_trimmomatic_kraken2
